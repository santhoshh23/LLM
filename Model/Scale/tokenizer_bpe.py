...
class BPETokenizer:
    ...
    def train_from_iterator(self, texts: list[str], vocab_size: int = None):
        """Train BPE tokenizer directly from an in-memory list of text strings."""
        if ByteLevelBPETokenizer is None:
            raise ImportError("Please `pip install tokenizers` for BPETokenizer.")
        self.vocab_size = vocab_size or self.vocab_size
        tok = ByteLevelBPETokenizer()
        tok.train_from_iterator(texts, vocab_size=self.vocab_size, special_tokens=self.special_tokens)
        self._tok = tok
